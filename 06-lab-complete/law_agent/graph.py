...
async def check_routing(state: LawState) -> dict:
    ...
    Uses keyword-based routing (no LLM call) for speed.
    ...
    needs_tax = any(kw in q for kw in [
        "tax", "irs", "evasion", "fbar", "fatca", "penalty", "penalties",
        "income", "withholding", "transfer pricing", "offshore",
    ])
    needs_compliance = any(kw in q for kw in [
        "compliance", "sec", "sox", "fcpa", "aml", "bribery", "gdpr",
        "ccpa", "regulation", "regulatory", "corrupt", "fraud",
    ])
    # Fallback: route to both if no keyword matched
    if not needs_tax and not needs_compliance:
        needs_tax, needs_compliance = True, True
